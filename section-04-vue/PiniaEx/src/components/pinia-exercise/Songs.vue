...
<!--   Ej. 1: En el repo de proyecto donde estéis trabajando nos creamos una carpeta "stores". Dentro creamos una store de Pinia llamada songs.js. Este archivo debe tener la estructura que hemos visto de una Pinia Store. Dentro de la store, en el state, vamos a incluir una key "mySongs" y setearla en un array vacío.
 ...
 <script setup>
    ...
    import { useSongStore } from '/Users/karen/Desktop/class-notes/section-04-vue/PiniaEx/src/stores/songs.js';
  
  
   /*  const addSongs = ()=>{
    useSongStore.mySongs.value.push(newSong.value);
    newSong.value = ref("");
    return {addSongs};
    } */
  ...
  <template>
   ...
   <!--  <ul v-if="useSongStore.mySongs.value.length">
    <li v-for="song in useSongStore.mySongs.value" :key="song">{{ song }}</li></ul> -->
    <p>Todavía no hay canciones en la store</p> 
          
    <input v-model="mySongs" type="text"><br>
    <button @click="addSongs">añade la cancion</button>
  </template>
